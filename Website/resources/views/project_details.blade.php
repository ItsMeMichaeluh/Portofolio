...
<div class="flex h-screen items-center justify-center relative overflow-hidden">
    <div class="fixed top-6 left-6 z-50">
        <a href="{{ route('school') }}" class="block bg-black/30 hover:bg-black/50 backdrop-blur-md p-3 rounded-full transition-all duration-300 transform hover:scale-110 shadow-lg group">
            <svg class="w-6 h-6 text-white group-hover:text-purple-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<div class="animated-bg min-h-screen">
    ...
    <div class="max-w-6xl mx-auto px-6 py-16 relative">
        ...
        <div class="relative overflow-hidden rounded-2xl shadow-2xl border border-slate-700/50">
            <div class="flex gap-4 snap-x snap-mandatory overflow-x-auto image-gallery scrollbar-hide p-6 bg-slate-900/60 backdrop-blur-lg animate__animated animate__fadeIn">
                @foreach($project->images as $index => $image)
                <div id="image-{{ $index }}" class="w-full md:w-1/2 lg:w-1/3 flex-shrink-0 snap-center p-2 transform transition duration-500 hover:scale-[1.02]">
            ...
            <div class="flex justify-center mt-6 mb-2 gap-2">
                @foreach($project->images as $index => $image)
                    <button onclick="scrollToImage({{ $index }})"
                            class="w-3 h-3 rounded-full bg-purple-500/50 hover:bg-purple-400 transition"></button>
                @endforeach
            </div>
        </div>
    </div>
